        ...
        return await new Promise((resolve, reject) => {
            ...
            const receive = (chunk) => {
                for (const character of chunk.toString('utf8')) {
                    ...
                    if (/^[A-Za-z0-9_-]$/.test(character) && value.length < 128)
                        value += character;
                }
            };
...
async function main() {
    if (process.platform !== 'win32')
        throw new Error('Run the companion on your Windows PC.');
    if (!stdin.isTTY || !stdout.isTTY)
        throw new Error('Run npm run companion in an interactive Windows terminal.');
    if (process.argv.length > 2)
        throw new Error('Run npm run companion without additional arguments. Enter the website and pairing code at its prompts.');
    const prompt = createInterface({ input: stdin, output: stdout });
    let origin;
    try {
        origin = validateCompanionOrigin(await prompt.question('Appraisal Desk HTTPS website (for example https://your-app.azurewebsites.net): '));
    }
    finally {
        prompt.close();
    }
