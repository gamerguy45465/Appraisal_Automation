...
import { COMPANION_TOKEN_PATTERN, relayExchangeInputSchema, serializeJobPayload } from './companion-protocol.js';
...
export function createRelayJobRunner() {
    const jobs = new Map();
    let connection;
    let stopped = false;
    const current = () => connection?.currentJobId ? jobs.get(connection.currentJobId) : undefined;
    const locked = () => Boolean(connection?.browserOpen || current() && !current().view.canStartAnother);
    const connected = () => Boolean(connection?.acceptsJobs && connection.bearerHash && connection.expiresAt > Date.now()
        && connection.lastSeen !== undefined && Date.now() - connection.lastSeen <= CONNECTED_WINDOW_MS);
    const ensureRunning = () => {
        if (stopped)
            throw new AppError('COMPANION_UNAVAILABLE', 'The companion relay is shutting down.', 503);
    };
    const cleanup = () => {
        const now = Date.now();
        if (connection?.pairingHash && connection.pairingExpiresAt <= now) {
            connection.pairingHash.fill(0);
            connection.pairingHash = undefined;
        }
    ...
    return {
        pair(owner) {
            ...
            cleanup();
            if (locked())
                throw new AppError('COMPANION_BUSY', 'Finish the current preparation and close its R3 browser before pairing another companion.', 409);
        ...
        create(owner, payload) {
            ...
            while (jobs.size >= 100) {
                ...
                if (!id)
                    throw new AppError('JOB_LIMIT', 'Restart the Azure session after finishing the current order.', 503);
                wipePayload(jobs.get(id));
                jobs.delete(id);
            }
            const id = randomUUID();
            const view = { id, status: 'queued', message: 'Your documents are queued for the Windows companion.', canStartAnother: false };
        ...
        exchange(bearer, input) {
            ...
            if (!parsed.success)
                throw new AppError('COMPANION_REQUEST_INVALID', 'The companion sent an invalid status update.', 400);
            const request = parsed.data;
            const job = current();
            if (request.sequence > connection.sequence) {
                const ids = [request.acceptedJobId, request.declinedJobId, request.update?.id].filter((id) => Boolean(id));
                if (ids.some(id => jobs.get(id)?.connection !== connection))
                    throw new AppError('COMPANION_JOB_REJECTED', 'This preparation does not belong to the companion.', 409);
                ...
                if (acceptsCurrent && (!job.delivered || job.declined))
                    throw new AppError('COMPANION_JOB_REJECTED', 'This preparation is not available for acceptance.', 409);
                ...
                if (job && (!job.delivered || job.declined || declinesCurrent) && request.update?.id === connection.lastAcceptedJobId) {
                    const update = request.update;
                    if (update && update.canStartAnother && readyStatuses.has(update.status) && !update.browserOpen)
                        connection.browserOpen = false;
                }
                if (acceptsCurrent) {
                    job.accepted = true;
                    connection.lastAcceptedJobId = job.view.id;
                    wipePayload(job);
                }
                if (declinesCurrent) {
                    job.declined = true;
