import { z } from 'zod';
import { inputSchema, MAX_PDF_BYTES, validatePdf } from './domain.js';
...
const serializedInputSchema = z.preprocess(value => {
    if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).some(key => !inputKeys.has(key)))
        return undefined;
    return value;
}, inputSchema);
...
export function deserializeJobPayload(value) {
    ...
    try {
        const decode = (base64, name) => {
            ...
            buffers.push(buffer);
            if (buffer.length > MAX_PDF_BYTES || buffer.toString('base64') !== base64)
...
//# sourceMappingURL=companion-protocol.js.map