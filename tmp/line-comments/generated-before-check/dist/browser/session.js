...
import { isAuthenticatedLandingUrl, isOrderConfirmationUrl, isOrderUrl, maySendRequest, R3_LOGIN_URL, R3_ORDER_URL, } from './guard.js';
...
export async function createGuardedSession(browser, context, page, options, testTransport) {
    let phase = 'preparing';
    ...
    await context.routeWebSocket('**/*', (socket) => {
        if (phase === 'review')
            ...
            socket.close({ code: 1008, reason: 'Human review is required before writes.' });
    });
    ...
    page.on('dialog', (dialog) => {
        if (phase !== 'review' && phase !== 'authenticating')
    ...
    return {
        ...
        isOrderPage: () => hasLoggedIn && phase !== 'authenticating' && isOrderUrl(page.url()),
        ...
        async close() { await browser.close(); onClosed(); },
    };
}
