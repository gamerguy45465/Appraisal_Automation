...
export interface AzureBrowserConfig {
  readonly serviceUrl: string;
  readonly managedIdentityClientId?: string;
}

/** Private server-to-worker data. Never include this value in status or model tools. */
export interface AzureBrowserConnection {
...
function validateConfig(serviceUrl: string | undefined, clientId: string | undefined): AzureBrowserConfig {
  let endpoint: URL;
  try { endpoint = new URL(serviceUrl ?? ''); } catch { throw configurationError(); }
  ...
  if (clientId !== undefined && (clientId.length !== 36 || !UUID.test(clientId))) throw configurationError();
...
export function resolveAzureBrowserConfig(env: NodeJS.ProcessEnv = process.env): AzureBrowserConfig {
 ...
 */
export function createAzureBrowserConnector(config: AzureBrowserConfig): () => Promise<AzureBrowserConnection> {
