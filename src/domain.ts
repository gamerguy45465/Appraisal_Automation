...
// [L7] Derives the provider union type from the supported provider list.
...
// [L21] Derives the validated order-input TypeScript type from the schema.
export type OrderInput = z.infer<typeof inputSchema>;
// [L22] Defines an uploaded PDF as a readonly filename and in-memory byte buffer.
export interface UploadedPdf { readonly name: string; readonly buffer: Buffer }
// [L23] Defines a readonly job payload containing validated input, a required URLA, and an optional sales contract.
export interface JobPayload { readonly input: OrderInput; readonly urla: UploadedPdf; readonly salesContract?: UploadedPdf }
// [L24] Defines the job lifecycle status values exposed by the application.
export type JobStatus = 'queued' | 'extracting' | 'awaiting_login' | 'preparing' | 'awaiting_review' | 'user_submitted' | 'browser_closed' | 'failed';
// [L25] Defines status updates with a message and optional warning list.
export interface JobUpdate { readonly status: JobStatus; readonly message: string; readonly warnings?: string[] }
// [L26] Adds the fixed status-message discriminator to worker updates.
...
// [L45] Derives the extracted-order TypeScript type from runtime validation.
export type ExtractedOrder = z.infer<typeof extractionSchema>;
// [L46] Defines readonly approved field-plan entries with a value, control kind, optional required flag, and approved label aliases.
