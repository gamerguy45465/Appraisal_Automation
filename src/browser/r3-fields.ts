...
const currentContactSections: Record<string, string> = {
...
export function r3ContactIdentity(field: { id: string; name?: string }): string | undefined {
...
export function matchesR3Field(fieldKey: string, field: { id: string; name?: string; section: string; label: string; tag: string; type: string; visible?: boolean }): boolean {
  ...
  if (expectedId && field.id === expectedId) return true;
  ...
  if (current) {
    if (rest.length || !Object.hasOwn(currentContactSections, section) || !Object.hasOwn(contactSuffixes, name)
      ...
      || !['input', 'text', 'email', 'tel'].includes(field.type)) return false;
    ...
    if (field.visible === false && section !== 'contact') return false;
  ...
  if (/^OrderItemEdit_Contacts/i.test(field.id) || /^OrderItemEdit\.Contacts/i.test(field.name ?? '')) return false;
