...
/** These actions are available only to the authenticated human viewer, never to model tools. */
export type HumanBrowserAction = z.infer<typeof actionSchema>;
export type HumanBrowserPhase = 'authenticating' | 'preparing' | 'review';
export interface HumanBrowserFrame {
  /** Base64 JPEG; empty only when a native dialog prevents the first capture. */
  image: string;
  width: number;
  height: number;
  canControl: boolean;
  phase: HumanBrowserPhase;
  pages: Array<{ id: string; title: string; selected: boolean }>;
  dialog?: { type: string; message: string; defaultValue?: string };
}
export interface HumanBrowserView {
  frame(): Promise<HumanBrowserFrame>;
  act(action: HumanBrowserAction): Promise<void>;
}
interface HumanBrowserState {
  phase: HumanBrowserPhase | 'closed';
  canControl: boolean;
  closed: boolean;
}

const unavailable = (): AppError => new AppError('HUMAN_BROWSER_UNAVAILABLE', 'The cloud browser is no longer available.', 409);
const paused = (): AppError => new AppError('HUMAN_BROWSER_PAUSED', 'Browser input is paused while automatic preparation is running.', 409);
const invalid = (): AppError => new AppError('HUMAN_BROWSER_INVALID', 'The browser action is invalid.', 400);
const actionFailed = (): AppError => new AppError('HUMAN_BROWSER_ACTION_FAILED', 'The browser action could not finish. Refresh the browser view and try again.', 409);

export function validateHumanBrowserAction(value: unknown): HumanBrowserAction {
...
export function createHumanBrowserView(context: BrowserContext, primaryPage: Page, state: () => HumanBrowserState): {
  view: HumanBrowserView;
  ...
  const nativeInputs = new Set<Promise<void>>();
  let selected = primaryPage;
  let framePromise: Promise<HumanBrowserFrame> | undefined;
  const pages = new Map<Page, { id: string; title: string; dialog?: Dialog; onDialog(dialog: Dialog): void; onReady(): void; onClose(): void }>();
  const images = new Map<Page, { image: string; width: number; height: number }>();

  const ensureOpen = (): HumanBrowserState => {
    const current = state();
    if (disposed || current.closed || current.phase === 'closed') throw unavailable();
    return current;
  };
  const ensureControl = (expectedGeneration: number): void => {
    const current = ensureOpen();
    if (suspended || generation !== expectedGeneration || !current.canControl
      || !['authenticating', 'review'].includes(current.phase)) throw paused();
  };
  ...
  const watchPage = (watched: Page): void => {
    if (disposed || watched.isClosed() || pages.has(watched)) return;
    const registration = {
      id: randomUUID(),
      title: 'R3 browser',
      ...
      onDialog(dialog: Dialog): void {
        ...
        } else registration.dialog = dialog;
      },
      onReady(): void {
        // Titles are optional labels. Never block a frame on a page evaluation behind a modal.
        if (!registration.dialog) void watched.title().then(title => { registration.title = title.slice(0, 200) || 'R3 browser'; }).catch(() => undefined);
      },
      onClose(): void {
  ...
  const capture = async (): Promise<HumanBrowserFrame> => {
    ensureOpen();
    ...
    if (!registration.dialog) {
      try {
        ...
        if (!registration.dialog) throw new AppError('HUMAN_BROWSER_FRAME_FAILED', 'The cloud browser image is unavailable. Try again shortly.', 409);
      }
    }
    ...
    return {
      ...captured,
      phase: current.phase as HumanBrowserPhase,
  ...
  const nativeInput = async (page: Page, action: () => Promise<void>): Promise<void> => {
    let notifyDialog!: () => void;
    const dialog = new Promise<void>(resolve => { notifyDialog = resolve; });
    ...
    try { await Promise.race([work, dialog]); }
    finally { page.off('dialog', notifyDialog); }
  };

  const view: HumanBrowserView = {
    frame() {
      framePromise ??= capture().finally(() => { framePromise = undefined; });
      return framePromise;
    },
    act(input) {
      let action: HumanBrowserAction;
      const actionGeneration = generation;
      try {
        action = validateHumanBrowserAction(input);
        ensureControl(actionGeneration);
        if (pendingActions >= 32) throw new AppError('HUMAN_BROWSER_BUSY', 'Wait for the current browser input to finish.', 429);
      } catch (error) { return Promise.reject(error instanceof AppError ? error : invalid()); }
      pendingActions++;
      const operation = queue.then(async () => {
        ensureControl(actionGeneration);
        if (action.type === 'select-page') {
          const match = [...pages.entries()].find(([, entry]) => entry.id === action.pageId)?.[0];
          if (!match || match.isClosed()) throw invalid();
          selected = match;
        ...
        if (action.type === 'dialog') {
          const dialog = registration.dialog;
          if (!dialog) throw invalid();
          if (action.accept) await dialog.accept(dialog.type() === 'prompt' ? action.promptText : undefined);
          else await dialog.dismiss();
          if (registration.dialog === dialog) registration.dialog = undefined;
          return;
        }
        if (registration.dialog) throw new AppError('HUMAN_BROWSER_DIALOG', 'Respond to the browser dialog before continuing.', 409);
        switch (action.type) {
          case 'click': {
            const size = page.viewportSize() ?? { width: 1440, height: 1000 };
            if (action.x >= size.width || action.y >= size.height) throw invalid();
            await nativeInput(page, () => page.mouse.click(action.x, action.y, { button: action.button ?? 'left' }));
            break;
          }
          case 'text': await nativeInput(page, () => page.keyboard.insertText(action.text)); break;
          case 'key': await nativeInput(page, () => page.keyboard.press(action.key)); break;
          case 'scroll': await nativeInput(page, () => page.mouse.wheel(action.deltaX, action.deltaY)); break;
        }
      }).catch((error: unknown) => { throw error instanceof AppError ? error : actionFailed(); });
  ...
  return {
    ...
    resume() { if (!disposed) { generation++; suspended = false; } },
    dispose() {
      if (disposed) return;
      disposed = true;
      suspended = true;
