...
// [L7] Defines the four browser lifecycle phases used by the request guard.
export type BrowserPhase = 'preparing' | 'authenticating' | 'review' | 'closed';
...
export function isOrderUrl(rawUrl: string): boolean {
...
export function isAuthenticatedLandingUrl(rawUrl: string): boolean {
...
function isChallengeRequest(url: URL, method: string, isMainFrameNavigation: boolean): boolean {
  // [L44] Rejects main-frame navigation and requests outside the reCAPTCHA path prefix.
  if (isMainFrameNavigation || !url.pathname.startsWith('/recaptcha/')) return false;
  // [L45] Allows only GET and HEAD for reCAPTCHA assets on Google's static-content origin.
  if (url.origin === 'https://www.gstatic.com') return method === 'GET' || method === 'HEAD';
...
export function maySendRequest(input: {
  // [L52] Requires the candidate request URL as a string.
  url: string;
  // [L53] Requires the request's HTTP method.
  method: string;
  // [L54] Records whether the request performs a document navigation.
  isNavigation: boolean;
  // [L55] Optionally identifies navigation of the main browser frame.
  isMainFrameNavigation?: boolean;
  // [L56] Optionally carries the request resource category.
  resourceType?: string;
  // [L57] Requires the current browser phase for phase-dependent decisions.
  phase: BrowserPhase;
// [L58] Completes the input type and begins the boolean authorization implementation.
}): boolean {
  ...
  if (input.phase === 'review') return true;
  ...
  if (input.phase === 'authenticating') {
    ...
    if (isChallengeRequest(url, method, input.isMainFrameNavigation ?? input.isNavigation)) return true;
    // [L75] Prevents the order creation page from opening while sign-in is still being confirmed.
    if (isOrderUrl(url.href)) return false;
  ...
  if (!input.isNavigation) {
    ...
    if (url.origin === 'https://clients.r3amc.com') {
      ...
      if (/^\/(bundles|Content|Scripts|fonts)\//i.test(path) || /^\/favicon\.(ico|png)$/i.test(path)) return true;
    ...
    if (url.origin === 'https://r3amc.com') return /^\/(wp-content|wp-includes)\//i.test(path);
  ...
  if (url.origin === 'https://r3amc.com') return url.pathname === '/' && !url.search;
  // [L95] Rejects navigation to any remaining origin outside the client portal.
  if (url.origin !== 'https://clients.r3amc.com') return false;
  // [L96] Normalizes a trailing slash and path case, then rejects paths outside the known navigation set.
  if (!allowedNavigationPaths.has(url.pathname.replace(/\/$/, '').toLowerCase() || '/')) return false;
  ...
  if (isLoginUrl(url.href)) {
    ...
    if (keys.some((key) => key.toLowerCase() !== 'returnurl')) return false;
...
export function isOrderConfirmationUrl(rawUrl: string): boolean {
