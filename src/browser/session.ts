...
// [L3] Imports the message-content type used by the screenshot tool's multimodal result.
import type { MessageContent } from '@langchain/core/messages';
// [L4] Imports Chromium launching plus Playwright browser, context, locator, page, request, and route types.
import { chromium, type Browser, type BrowserContext, type Dialog, type Locator, type Page, type Request, type Response, type Route, type WebSocketRoute } from 'playwright';
...
import {
  ...
  R3_AUTHENTICATED_URL, R3_LOGIN_URL, R3_ORDER_URL, type BrowserPhase,
...
// [L13] Defines the public shape of an application-approved field assignment.
export interface FieldPlanEntry {
  // [L14] Identifies the application field-plan key.
  key: string;
  // [L15] Allows an approved string value or boolean checkbox value.
  value: string | boolean;
  // [L16] Restricts field actions to text entry, dropdown selection, or checkbox/radio setting.
  kind: 'text' | 'select' | 'checkbox';
  // [L17] Optionally marks the field as required for a completed handoff.
  required?: boolean;
  // [L18] Documents that alternate labels must be explicitly approved by application code.
  /** Explicit aliases approved by the application, never invented by the agent. */
  // [L19] Allows a list of application-approved alternate dropdown labels.
  allowedLabels?: string[];
// [L20] Ends the approved field-entry interface.
}
// [L21] Blank line separating the surrounding declarations, statements, or document blocks.

// [L22] Defines the inspected form-element data returned to browser tools.
export interface FormElement {
  // [L23] Stores the opaque session-generated element reference.
  ref: string;
  // [L24] Stores the observed human-readable field label.
  label: string;
  // [L25] Stores the observed containing-section hierarchy.
  section: string;
  // [L26] Stores the element's form name.
  name: string;
  // [L27] Stores the DOM element ID for inspected identity matching.
  id: string;
  // [L28] Stores the lowercased HTML tag name.
  tag: string;
  // [L29] Stores the input type, ARIA role, or fallback tag classification.
  type: string;
  // [L30] Stores the observed field value as text.
  value: string;
  // [L31] Stores checkbox/radio state, using null for other control types.
  checked: boolean | null;
  // [L32] Indicates whether HTML or ARIA metadata marks the field required.
  required: boolean;
  // [L33] Indicates whether HTML or ARIA metadata disables the control.
  disabled: boolean;
  // [L34] Indicates whether HTML or ARIA metadata marks the control read-only.
  readOnly: boolean;
  // [L35] Records the element's measured visibility.
  visible: boolean;
  // [L36] Stores native dropdown options with their labels, raw values, and disabled flags.
  options: Array<{ label: string; value: string; disabled: boolean }>;
// [L37] Ends the inspected form-element interface.
}
// [L38] Blank line separating the surrounding declarations, statements, or document blocks.

// [L39] Defines the result of checking one planned field against the page.
export interface FieldVerification {
  // [L40] Identifies which approved plan key was checked.
  fieldKey: string;
  // [L41] Identifies the inspected DOM reference associated with the check.
  ref: string;
  // [L42] Carries the observed field label into the report.
  label: string;
  // [L43] Carries the observed section hierarchy into the report.
  section: string;
  // [L44] Stores the approved string or boolean expected value.
  expected: string | boolean;
  // [L45] Stores the actual string or boolean read back from the page.
  actual: string | boolean;
  // [L46] Uses the same control-kind type as an approved field entry.
  kind: FieldPlanEntry['kind'];
  // [L47] Records whether the current page value matches the approved plan.
  verified: boolean;
  // [L48] Optionally explains a failed verification or other field issue.
  message?: string;
// [L49] Ends the field-verification result interface.
}
// [L50] Blank line separating the surrounding declarations, statements, or document blocks.

// [L51] Defines browser-session configuration supplied by the application.
...
export interface BrowserSession {
  ...
  isOrderPage(): boolean;
  isClosed(): boolean;
  getFieldReport(): Promise<FieldVerification[]>;
  fillApprovedPlan(): Promise<{ report: FieldVerification[]; errors: Array<{ fieldKey: string; message: string }> }>;
  handoff(): Promise<void>;
  handoffIncomplete(): Promise<boolean>;
  startNextOrder(options: BrowserSessionOptions): Promise<BrowserSession>;
  waitUntilClosed(): Promise<void>;
  close(): Promise<void>;
}
// [L54] Blank line separating the surrounding declarations, statements, or document blocks.

// [L55] Defines the status, optional headers, and string/buffer body returned by guarded network transport.
export interface GuardedResponse { status: number; headers?: Record<string, string>; body: string | Buffer }
// [L56] Defines an injectable async route transport, allowing deterministic network fixtures.
export type GuardedTransport = (route: Route) => Promise<GuardedResponse>;
// [L57] Defines the redirect-related metadata read from Chromium's native response events.
interface NativeResponseMetadata {
  // [L58] Stores Chromium's identifier for the paused request.
  requestId: string;
  // [L59] Optionally stores the response HTTP status code.
  responseStatusCode?: number;
  // [L60] Optionally stores response headers as name/value pairs.
  responseHeaders?: Array<{ name: string; value: string }>;
  // [L61] Stores the request URL and HTTP method needed for redirect validation.
  request: { url: string; method: string };
  // [L62] Stores Chromium's resource classification for navigation checks.
  resourceType: string;
  // [L63] Stores the frame identifier used to distinguish main-frame navigation.
  frameId: string;
// [L64] Ends the native-response metadata interface.
}
...
// [L71] Ends the browser action error class.
}
...
function hasAuthenticatedAccountMarker(expectedUrl: string): boolean {
  const current = new URL(location.href);
  const expected = new URL(expectedUrl);
  if (current.origin !== expected.origin || current.pathname.toLowerCase() !== expected.pathname.toLowerCase()
    || current.username || current.password || current.search) return false;
  ...
  for (let ancestor: Element | null = logout; ancestor; ancestor = ancestor.parentElement) {
    if (ancestor.matches('[hidden], [inert], [aria-hidden="true" i]')) return false;
  }
  ...
  if (item?.tagName !== 'LI' || !menu?.matches('ul.dropdown-menu.dropdown-navbar-menu')
    ...
    || !visible(owner) || !visible(navbar) || getComputedStyle(menu).display !== 'none') return false;
...
// [L75] Documents the visible, isolated, nonpersistent authentication design of production sessions.
interface BrowserOwnerRegistration {
  permitsReview(): boolean;
  onClosed(): void;
}

interface BrowserOwner {
  active?: BrowserOwnerRegistration;
  isClosed(): boolean;
...
const browserOwners = new WeakMap<BrowserContext, Promise<BrowserOwner>>();

function getBrowserOwner(browser: Browser, context: BrowserContext, ownsBrowser: boolean): Promise<BrowserOwner> {
  const existing = browserOwners.get(context);
  if (existing) return existing;
  const creating = (async (): Promise<BrowserOwner> => {
    let closed = false;
    let resolveClosed!: () => void;
    let shutdown: Promise<void> | undefined;
    const lifetime = new Promise<void>(resolve => { resolveClosed = resolve; });
    const sockets = new Set<{ close(): Promise<void> }>();
    const watchedPages = new Map<Page, () => void>();
    const shutdownBrowser = (): Promise<void> => shutdown ??= browser.close();
    ...
    const finishClosed = (): void => {
      ...
      for (const [watched, listener] of watchedPages) watched.off('close', listener);
      watchedPages.clear();
      void Promise.allSettled([closeSockets(), ...(ownsBrowser ? [shutdownBrowser()] : [])]).then(() => {
        resolveClosed();
      });
    };
    const watchPage = (watched: Page): void => {
      if (watchedPages.has(watched)) return;
      const onPageClosed = (): void => {
        watchedPages.delete(watched);
        if (context.pages().length === 0) finishClosed();
      };
    ...
    await context.routeWebSocket('**/*', socket => {
      ...
      const connection = {
        close: (): Promise<void> => closing ??= Promise.resolve().then(() => Promise.allSettled([
      ...
      const forward = (destination: WebSocketRoute, message: string | Buffer): void => {
        if (closed || closing || owner.active !== registration || !registration.permitsReview()) {
          void connection.close();
          return;
        }
        try { destination.send(message); } catch { void connection.close(); }
      };
...
export async function createGuardedSession(
  ...
  ownsBrowser = false,
// [L94] Completes the parameters and begins guarded-session initialization.
  ...
  const contactIdentities = new Map<string, string>();
  ...
  const routedRequests = new Map<Route, Promise<void>>();
...
// [L147] Blank line separating the surrounding declarations, statements, or document blocks.
  ...
  const handleRequest = async (route: Route): Promise<void> => {
  ...
  const routeHandler = async (route: Route): Promise<void> => {
    ...
    try { await work; }
    finally { routedRequests.delete(route); }
  };
  ...
  const onDialog = (dialog: Dialog): void => {
    ...
    if (transferring && dialog.type() === 'beforeunload') void dialog.accept().catch(() => undefined);
    else if (phase !== 'review' && phase !== 'authenticating') void dialog.dismiss().catch(() => undefined);
...
// [L218] Blank line separating the surrounding declarations, statements, or document blocks.
  ...
  const ensureAvailable = (): void => {
    ...
    if (browserOwner.isClosed() || phase === 'closed' || page.isClosed()) throw new BrowserActionError('The appraisal browser is closed.');
...
// [L308] Blank line separating the surrounding declarations, statements, or document blocks.
  ...
  function fieldMatches(fieldKey: string, fields: FormElement[]): FormElement[] {
    const role = fieldKey.split('.')[0]!;
    const contactFields = ['firstName', 'lastName', 'workPhone', 'homePhone', 'mobilePhone', 'email'];
    const canRead = (field: FormElement): boolean => {
      if (!r3ContactIdentity(field) || field.visible) return true;
      if (role !== 'contact' || !borrowerCopyVerified || approvedPlan.get('borrowerIsAccessContact')?.value !== true) return false;
  ...
  function getPlan(fieldKey: string, ref: string, kind: FieldPlanEntry['kind']): FieldPlanEntry {
    ...
    if (identity) contactIdentities.set(fieldKey.split('.')[0]!, identity);
...
// [L402] Blank line separating the surrounding declarations, statements, or document blocks.
  ...
  async function applyField(fieldKey: string, ref: string, kind: FieldPlanEntry['kind']): Promise<FieldVerification> {
    ...
    if (kind === 'text') {
      ...
      if (fieldKey.startsWith('borrower.')) borrowerCopyVerified = false;
...
// [L607] Blank line separating the surrounding declarations, statements, or document blocks.
  ...
  const removeSessionObservers = (): void => {
  ...
  const startNextOrder = async (nextOptions: BrowserSessionOptions): Promise<BrowserSession> => {
    if (browserOwner.isClosed()) throw new BrowserActionError('The appraisal browser is closed.');
    if (page.isClosed()) throw new AppError('BROWSER_REUSE_UNAVAILABLE', 'The prior order tab was closed. Start the next order in a fresh browser.');
  ...
  return {
    ...
    async waitForUserLogin({ signal }: { signal?: AbortSignal } = {}): Promise<void> {
      ...
      loginWait = (async () => {
        ...
        let loginPageStatus: number | undefined;
        ...
        const continueNativeResponse = (event: NativeResponseMetadata): void => {
          ...
          void (async () => {
            ...
            if (event.resourceType === 'Document' && event.frameId === frameTree.frame.id && event.request.method === 'GET') loginPageStatus = status;
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          const failOpening = (): void => fail(loginPageStatus !== undefined && loginPageStatus >= 400
    ...
    isOrderPage: (): boolean => !retired && !browserOwner.isClosed() && !page.isClosed() && hasLoggedIn && phase !== 'authenticating' && isOrderUrl(page.url()),
    ...
    handoffIncomplete(): Promise<boolean> {
      ...
      if (retired || closedResolved || page.isClosed()) return Promise.resolve(false);
