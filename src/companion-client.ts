...
import type { JobView } from './domain.js';
import type { JobRunner } from './jobs.js';
import {
  deserializeJobPayload, relayConnectResponseSchema, relayExchangeInputSchema,
  relayExchangeResponseSchema, COMPANION_TOKEN_PATTERN, MAX_RELAY_RESPONSE_BYTES, type RelayExchangeInput,
} from './companion-protocol.js';

const REQUEST_TIMEOUT_MS = 30_000;
const MAX_ACCEPTED_JOBS = 1000;

type ConnectionProblem = 'origin' | 'pairing' | 'pairing_connection' | 'connection' | 'authorization' | 'protocol' | 'busy' | 'limit';
const problemMessages: Record<ConnectionProblem, string> = {
...
export class CompanionConnectionError extends Error {
  constructor(readonly problem: ConnectionProblem) {
    super(problemMessages[problem]);
    this.name = 'CompanionConnectionError';
  }
}

export function validateCompanionOrigin(value: string): string {
  try {
    const origin = value.trim();
    const parsed = new URL(origin);
    if (parsed.protocol !== 'https:' || parsed.username || parsed.password || parsed.search || parsed.hash
      || (origin !== parsed.origin && origin !== `${parsed.origin}/`)) throw new Error();
    return parsed.origin;
  } catch { throw new CompanionConnectionError('origin'); }
}
...
async function readBoundedJson(response: Response, maximumBytes: number): Promise<unknown> {
  ...
  if (!reader) throw new CompanionConnectionError('protocol');
  const chunks: Buffer[] = [];
  let bytes = 0;
  try {
    while (true) {
    ...
    try { return JSON.parse(combined.toString('utf8')) as unknown; }
    catch { throw new CompanionConnectionError('protocol'); }
...
export interface CompanionClient {
  ...
  exchangeOnce(): Promise<void>;
  run(): Promise<void>;
  requestStop(): void;
  hasActiveBrowser(): boolean;
}

const stageMessages: Record<JobView['status'], string> = {
...
export function createCompanionClient(options: CompanionClientOptions): CompanionClient {
  ...
  if (!COMPANION_TOKEN_PATTERN.test(pairingToken)) throw new CompanionConnectionError('pairing');
  const fetchRequest = options.fetch ?? fetch;
  const runner = options.runner;
  if (typeof runner.getActive !== 'function') throw new CompanionConnectionError('protocol');
  ...
  let currentCloudId: string | undefined;
  let declinedCloudId: string | undefined;
  let sequence = 0;
  ...
  const hasActiveBrowser = (): boolean => Boolean(runner.getActive?.(owner));
  const getCurrentView = (): JobView | undefined => {
    const localId = currentCloudId && acceptedJobs.get(currentCloudId);
    return localId ? runner.get(owner, localId) : undefined;
  };
  const request = async (path: '/api/companion/connect' | '/api/companion/exchange', credential: string, body: unknown, limit: number): Promise<unknown> => {
    try {
      ...
      if (!response.ok) {
        await response.body?.cancel();
        if (response.status === 401 || response.status === 403) throw new CompanionConnectionError('authorization');
        if (response.status === 429 || response.status >= 500) throw new CompanionConnectionError('connection');
        throw new CompanionConnectionError('protocol');
      }
      return await readBoundedJson(response, limit);
    } catch (error) {
      if (error instanceof CompanionConnectionError) throw error;
  ...
  const client: CompanionClient = {
    ...
    async connect() {
      if (token) return;
      if (!pairingToken || connectionEnded) throw new CompanionConnectionError('authorization');
      try {
        const response = relayConnectResponseSchema.safeParse(await request('/api/companion/connect', pairingToken, {}, 4096));
        if (!response.success) throw new CompanionConnectionError('protocol');
        token = response.data.token;
      } catch (error) {
        if (error instanceof CompanionConnectionError && error.problem === 'connection') throw new CompanionConnectionError('pairing_connection');
        throw error;
      } finally {
    ...
    async exchangeOnce() {
      if (!token || connectionEnded) throw new CompanionConnectionError('authorization');
      if (exchanging) throw new CompanionConnectionError('busy');
      exchanging = true;
      try {
        ...
        const body: RelayExchangeInput = relayExchangeInputSchema.parse({
        ...
        if (stopping) {
          ...
          if (!acceptedJobs.has(delivery.id)) declinedCloudId = delivery.id;
          return;
        }
        if (acceptedJobs.has(delivery.id)) {
          // Old responses cannot replace a newer order's identity or restart paid extraction.
          if (delivery.id !== currentCloudId) throw new CompanionConnectionError('protocol');
          return;
        }
        if (acceptedJobs.size >= MAX_ACCEPTED_JOBS) throw new CompanionConnectionError('limit');
        let payload;
        try { payload = deserializeJobPayload(delivery.payload); }
        catch { throw new CompanionConnectionError('protocol'); }
        try {
    ...
    requestStop() {
      if (stopping) return;
      stopping = true;
      if (hasActiveBrowser()) tell('No new orders will start. Keep this terminal open while finishing the R3 form; the companion will exit after you close its browser.');
    },
    async run() {
      await client.connect();
      tell('Windows companion connected. Keep this terminal open while using Appraisal Desk.');
      let lastProblem: ConnectionProblem | undefined;
      while (true) {
        const currentView = getCurrentView();
        const stage = currentView && `${currentCloudId}:${currentView.status}`;
        if (currentView && stage !== lastStage) { tell(stageMessages[currentView.status]); lastStage = stage; }
        ...
        if (!connectionEnded) {
          try {
            ...
            exchanged = true;
            if (lastProblem) tell('Website connection restored. Existing local preparation was not restarted.');
            lastProblem = undefined;
          } catch (error) {
            const safe = error instanceof CompanionConnectionError ? error : new CompanionConnectionError('connection');
            if (safe.problem !== lastProblem) tell(safe.message);
            lastProblem = safe.problem;
            if (!['connection', 'busy'].includes(safe.problem)) { connectionEnded = true; stopping = true; token = undefined; }
          }
        }
        if ((finished && exchanged && !declinedCloudId) || (connectionEnded && !hasActiveBrowser())) return;
