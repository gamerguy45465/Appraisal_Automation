/** A listener is always a TCP port or a validated local Windows named pipe. */
export type HostingListener =
  | { readonly port: number; readonly host: '127.0.0.1' | '0.0.0.0' }
  | { readonly path: string };

interface HostingBase {
  readonly publicOrigin: string;
  readonly allowedOrigins: readonly string[];
  readonly allowedHosts: readonly string[];
}

/** Hosted mode never inherits localhost's origin or insecure-cookie allowances. */
export type HostingConfig = HostingBase & (
  | { readonly mode: 'local'; readonly listen: { readonly port: number; readonly host: '127.0.0.1' }; readonly secureCookies: false }
...
function parsePort(value: string | undefined): number {
  ...
  if (value.trim() !== value || !/^[0-9]{4,5}$/.test(value)) throw new Error(PORT_ERROR);
  const port = Number(value);
  if (port < 1024 || port > 65535) throw new Error(PORT_ERROR);
  return port;
}

/** Resolve only deployment-owned environment values, never request headers. */
export function resolveHosting(env: NodeJS.ProcessEnv = process.env): HostingConfig {
  ...
  if (originValue === undefined) throw new Error('Hosted mode requires APPRAISAL_PUBLIC_ORIGIN to identify the exact public HTTPS address.');
  const origin = parsePublicOrigin(originValue);
  const listen: HostingListener = env.PORT !== undefined && env.PORT.trim() === env.PORT && PIPE_PATTERN.test(env.PORT)
    ? { path: env.PORT }
