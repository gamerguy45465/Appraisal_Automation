...
interface AppOptions { readonly runner?: JobRunner; readonly port?: number }
...
export function createApp(options: AppOptions = {}) {
  ...
  app.use((req, _res, next) => {
    ...
    if (!origins.has(`http://${req.get('host')}`)) return next(new AppError('HOST_REJECTED', 'Open this application using its localhost address.', 403));
    ...
    if ((origin && !origins.has(origin)) || req.get('sec-fetch-site') === 'cross-site') return next(new AppError('ORIGIN_REJECTED', 'This request must come from the local application.', 403));
  ...
  app.use('/api', (req, res, next) => {
    ...
    if (!session || Date.now() - session.createdAt > 86400000) return next(new AppError('SESSION_REQUIRED', 'Refresh the page to start a local session.', 401));
