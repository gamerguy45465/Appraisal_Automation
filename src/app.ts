...
import { resolveHosting, type HostingConfig } from './hosting.js';
...
export function createApp(options: AppOptions = {}) {
  ...
  app.use((req, _res, next) => {
    ...
    if ((origin && !origins.has(origin)) || req.get('sec-fetch-site') === 'cross-site') return next(new AppError('ORIGIN_REJECTED', 'This request must come from the application website.', 403));
  ...
  const pruneSessions = (): void => {
    for (const [id, session] of sessions) if (Date.now() - session.createdAt > 86400000) sessions.delete(id);
  };
  ...
  app.use('/api', (req, res, next) => {
    ...
    if (!session || Date.now() - session.createdAt > 86400000) return next(hosted ? workspaceLoginRequired() : new AppError('SESSION_REQUIRED', 'Refresh the page to start a local session.', 401));
