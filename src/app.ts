...
import { DEFAULT_MODEL, DEFAULT_MODELS, DEFAULT_PROVIDER, inputSchema, MAX_PDF_BYTES, validatePdf, type UploadedPdf } from './domain.js';
...
// [L13] Defines an in-memory session record containing its CSRF token and creation timestamp.
interface Session { readonly token: string; readonly createdAt: number }
// [L14] Defines optional application dependencies for injecting a job runner and choosing the local port.
...
export function createApp(options: AppOptions = {}) {
  ...
  const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: MAX_PDF_BYTES, files: 2, fields: 10, fieldSize: 2048, parts: 12 },
    ...
    fileFilter: (_req, file, cb) => { if (file.mimetype !== 'application/pdf' || !file.originalname.toLowerCase().endsWith('.pdf')) cb(new AppError('INVALID_PDF', 'Upload PDF documents only.')); else cb(null, true); },
  ...
  app.use((req, _res, next) => {
    ...
    if (!hosts.has(req.get('host') ?? '')) return next(new AppError('HOST_REJECTED', hosted ? 'Open this application using its configured HTTPS address.' : 'Open this application using its localhost address.', 403));
    ...
    if ((origin !== undefined && !origins.has(origin)) || (req.get('sec-fetch-site') === 'cross-site' && !publicNavigation)) return next(new AppError('ORIGIN_REJECTED', 'This request must come from the application website.', 403));
  ...
  const cookieOptions = { httpOnly: true, sameSite: 'strict' as const, secure: hosting.secureCookies, path: '/', maxAge: 86400000 };
  const pruneSessions = (): void => {
    for (const [id, session] of sessions) if (Date.now() - session.createdAt > 86400000) { sessions.delete(id); runner.endOwner?.(id); }
  };
  const newSession = (res: Response): string => {
    pruneSessions();
    if (sessions.size >= 500) throw new AppError('SESSION_LIMIT', 'The workspace has reached its session limit. Try again later.', 503);
  ...
  const rateLimited = (_req: Request, res: Response): void => { res.status(429).json({ error: { code: 'RATE_LIMITED', message: 'Too many requests. Wait a minute and try again.' } }); };
  // Single-user buckets avoid trusting client-supplied proxy IP headers.
  const globalLimit = (limit: number, windowMs = 60000) => rateLimit({ windowMs, limit, keyGenerator: () => 'workspace', standardHeaders: 'draft-8', legacyHeaders: false, handler: rateLimited });
  ...
  if (hosted) {
    app.post('/api/login', globalLimit(10, 15 * 60000), express.json({ limit: '2kb' }), (req, res) => {
      ...
      if (req.get('origin') !== hosting.publicOrigin) throw new AppError('ORIGIN_REJECTED', 'Sign in from the application website.', 403);
      const key: unknown = req.body?.accessKey;
      req.body = {};
      if (typeof key !== 'string' || key.length > 256 || !timingSafeEqual(createHash('sha256').update(key).digest(), accessDigest!)) throw workspaceLoginRequired();
      pruneSessions();
  ...
  app.use('/api', (req, res, next) => {
    if (/^\/jobs\/[^/]+\/browser(?:\/|$)/.test(req.path)) viewerLimit(req, res, next);
    else ordinaryLimit(req, res, next);
  });
  ...
  app.get('/api/session', (req, res) => {
    ...
    let id: string = typeof req.cookies.appraisalSession === 'string' ? req.cookies.appraisalSession : '';
    ...
    if (!session) {
      if (hosted) throw workspaceLoginRequired();
      // [L42] Rejects creation once 500 sessions remain, asking the user to restart the local application.
      if (sessions.size >= 500) throw new AppError('SESSION_LIMIT', 'Restart the local application to clear unused sessions.', 503);
      // [L43] Generates independent random 32-byte hexadecimal session and CSRF tokens, timestamps the session, and stores it.
      id = randomBytes(32).toString('hex'); session = { token: randomBytes(32).toString('hex'), createdAt: Date.now() }; sessions.set(id, session);
  ...
  app.use('/api', (req, res, next) => {
    ...
    const id = req.cookies.appraisalSession as unknown;
    ...
    if (!session || Date.now() - session.createdAt > 86400000) { if (typeof id === 'string') runner.endOwner?.(id); return next(hosted ? workspaceLoginRequired() : new AppError('SESSION_REQUIRED', 'Refresh the page to start a local session.', 401)); }
    ...
    if (!['GET', 'HEAD'].includes(req.method)) {
      ...
      if (tokenBytes.length !== expectedBytes.length || !timingSafeEqual(tokenBytes, expectedBytes)) return next(new AppError('CSRF_REJECTED', 'Refresh the page before submitting again.', 403));
  ...
  app.post('/api/jobs', upload.fields([{ name: 'urla', maxCount: 1 }, { name: 'salesContract', maxCount: 1 }]), (req, res) => {
    ...
    try {
      ...
      if (!urlaFile) throw new AppError('URLA_REQUIRED', 'Upload the URLA PDF to continue.');
      // [L69] Assigns the URLA a fixed internal filename and validates the uploaded PDF bytes.
      const urla: UploadedPdf = { name: 'urla.pdf', buffer: urlaFile.buffer }; validatePdf(urla);
      ...
      if (salesContract) validatePdf(salesContract);
      // [L72] Creates a job owned by the validated local session with the parsed input and document buffers.
      const job = runner.create(res.locals.owner as string, { input, urla, salesContract });
    ...
    // [L74] Starts the failure path for input validation, PDF validation, or job creation errors.
      ...
      urlaFile?.buffer.fill(0); contractFile?.buffer.fill(0); throw error;
  ...
  app.get('/api/jobs/:id', (req, res) => {
    ...
    const job = runner.get(res.locals.owner as string, req.params.id);
  ...
  if (browserMode === 'azure') {
    const requireBrowser = (owner: string, id: string): void => {
      if (!/^[0-9a-f-]{36}$/.test(id) || !runner.get(owner, id)) throw new AppError('JOB_NOT_FOUND', 'This preparation was not found in your session.', 404);
    };
    app.get('/api/jobs/:id/browser', async (req, res) => {
      const owner = res.locals.owner as string;
      requireBrowser(owner, req.params.id);
      if (!runner.browserFrame) throw new AppError('BROWSER_UNAVAILABLE', 'The cloud browser is not available.', 409);
    ...
    app.post('/api/jobs/:id/browser/input', express.json({ limit: '32kb' }), async (req, res) => {
      const owner = res.locals.owner as string;
      requireBrowser(owner, req.params.id);
      if (req.get('origin') !== hosting.publicOrigin) throw new AppError('ORIGIN_REJECTED', 'Use the browser viewer in this workspace.', 403);
      const action = validateHumanBrowserAction(req.body);
      req.body = {};
      if (!runner.browserInput) throw new AppError('BROWSER_UNAVAILABLE', 'The cloud browser is not available.', 409);
    ...
    app.post('/api/jobs/:id/browser/close', async (req, res) => {
      ...
      requireBrowser(owner, req.params.id);
      if (req.get('origin') !== hosting.publicOrigin) throw new AppError('ORIGIN_REJECTED', 'Use the browser viewer in this workspace.', 403);
      if (!runner.closeBrowser) throw new AppError('BROWSER_UNAVAILABLE', 'The cloud browser is not available.', 409);
  ...
  const handleError: ErrorRequestHandler = (error: unknown, _req, res, _next) => {
    ...
    if (bodyError?.type === 'entity.too.large') { res.status(413).json({ error: { code: 'REQUEST_TOO_LARGE', message: 'The request is too large.' } }); return; }
    if (bodyError?.type === 'entity.parse.failed') { res.status(400).json({ error: { code: 'INVALID_REQUEST', message: 'The request could not be read.' } }); return; }
    // [L86] Converts Zod failures into HTTP 400 field-level JSON errors, joining nested paths with dots, then stops processing.
    if (error instanceof ZodError) { res.status(400).json({ error: { code: 'VALIDATION_ERROR', message: 'Check the highlighted form fields.', details: error.issues.map(i => ({ field: i.path.join('.'), message: i.message })) } }); return; }
    // [L87] Converts multer failures into a fixed HTTP 400 upload-limit response and stops processing.
    if (error instanceof multer.MulterError) { res.status(400).json({ error: { code: 'UPLOAD_LIMIT', message: 'Use one URLA and an optional sales contract, each a PDF no larger than 15 MiB.' } }); return; }
