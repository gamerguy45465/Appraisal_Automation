// [L1] Imports compile-time types JobPayload, WorkerUpdate, JobStatus from ./domain.js for shared validated application data, business rules, defaults, and domain types.
import type { JobPayload, WorkerUpdate, JobStatus } from './domain.js';
...
export async function runPreparation(payload: JobPayload, options: {
  // [L26] Requires a callback for sending typed worker status updates to the parent process.
  onStatus: (update: WorkerUpdate) => void;
  // [L27] Requires an external abort signal for stopping automation.
  signal: AbortSignal;
...
// [L28] Completes the options type and starts the asynchronous preparation implementation.
  ...
  const sendStatus = (status: JobStatus, message: string, warnings?: string[]): void => onStatus({ type: 'status', status, message, warnings });
  ...
  try {
    ...
    payload.urla.buffer.fill(0); payload.salesContract?.buffer.fill(0);
    ...
    const browserOptions: BrowserSessionOptions = { ...options.browserOptions, onStatus: (status, message) => {
      ...
      if (status !== 'awaiting_review') sendStatus(status, message, warnings);
    ...
    if (options.previousSession && !options.previousSession.isClosed()) {
      try { session = await options.previousSession.startNextOrder(browserOptions); }
      catch (error) {
        signal.throwIfAborted();
        ...
        if (!options.previousSession.isClosed()) throw error;
    ...
    if (!options.retainBrowser) await session.waitUntilClosed();
  // [L84] Begins failure handling while preserving any opened browser for review.
    ...
    timeout.clear(); controller.abort(); environment.clear();
    ...
    // [L89] Uses incomplete manual handoff when a browser session exists and remains available.
      ...
      try { released = await session.handoffIncomplete(); } catch { /* Keep the browser visible even if R3 is unavailable. */ }
