...
import { z } from 'zod';
// [L4] Imports compile-time types AppraisalModel from ./extraction.js for provider model construction and document extraction abstractions.
import type { AppraisalModel } from './extraction.js';
...
// [L7] Imports compile-time types createEnvironmentTools from ./environment.js for isolated per-job environment storage and tools.
import type { createEnvironmentTools } from './environment.js';
// [L8] Imports compile-time types BrowserSession from ./browser/session.js for the restricted R3 browser session and approved browser actions.
import type { BrowserSession } from './browser/session.js';
// [L9] Imports compile-time types AiProvider, FieldPlan from ./domain.js for shared validated application data, business rules, defaults, and domain types.
import type { AiProvider, FieldPlan } from './domain.js';
// [L10] Blank line separating the surrounding declarations, statements, or document blocks.

// [L11] Defines the dependencies and approved data required by the preparation agent.
interface AgentOptions {
  // [L12] Requires a read-only reference to the selected provider's chat model.
  readonly model: AppraisalModel;
  // [L13] Requires the provider identifier used to adapt model messages.
  readonly provider: AiProvider;
  // [L14] Requires the environment helper and its private input storage tools.
  readonly environment: ReturnType<typeof createEnvironmentTools>;
  // [L15] Requires the browser session exposing approved form actions.
  readonly session: BrowserSession;
  // [L16] Requires the validated field entries the agent may inspect and fill.
  readonly plan: FieldPlan[];
  // [L17] Requires the cancellation signal for stopping the agent invocation.
  readonly signal: AbortSignal;
// [L18] Closes the scope or expression introduced here: Defines the dependencies and approved data required by the preparation agent.
}
...
export async function prepareOrderWithAgent({ model, provider, environment, session, plan, signal }: AgentOptions): Promise<void> {
