# Line explanations: src/browser/r3-fields.ts

Source: [src/browser/r3-fields.ts](../../../../src/browser/r3-fields.ts). Numbers refer to the original file before comments were added.
...
September 14 contact repair: current R3 contacts use per-form UUID collection IDs, matching submitted names, exact role headings and field captions. `r3ContactIdentity` validates the ID/name identity; `matchesR3Field` permits only known contact suffixes and roles and rejects malformed Contacts namespaces before legacy fallback. Current fields require visibility except access copies subject to additional session guards. Historical exact IDs and reserved-ID restrictions remain. The current contact resolver supersedes historical statements below that only agent/officer roles use captions; no historical line numbers are assigned to the additions.
...
| 73 | Blank line separating the surrounding declarations, statements, or document blocks. |
...
| 104 | Blank line separating the surrounding declarations, statements, or document blocks. |
