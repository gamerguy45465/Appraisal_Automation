# Line explanations: src/errors.ts

Source: [src/errors.ts](../../../src/errors.ts). Numbers refer to the original file before comments were added.
...
| ---: | --- |
| 1 | Imports compile-time types AiProvider from ./domain.js for shared validated application data, business rules, defaults, and domain types. |
| 2 | Blank line separating the surrounding declarations, statements, or document blocks. |
| 3 | Existing explanatory comment: An intentionally safe message that can cross the API/worker boundary. |
...
| 8 | Closes the scope or expression introduced here: Creates an error with a read-only public code and HTTP status, defaulting the status to 400. |
| 9 | Closes the scope or expression introduced here: Defines the application error class used to carry deliberately public messages and error metadata. |
...
| 13 | Closes the scope or expression introduced here: Defines a helper for safely accessing properties of unknown error-like values. |
| 14 | Blank line separating the surrounding declarations, statements, or document blocks. |
| 15 | Existing explanatory comment: Inspect known SDK error shapes privately; none of this text may leave this module. |
...
| 21 | Closes the scope or expression introduced here: Defines private extraction of a provider message from known nested SDK error shapes. |
...
| 26 | Closes the scope or expression introduced here: Detects message patterns indicating insufficient Anthropic API credits. |
...
| 29 | Closes the scope or expression introduced here: Detects Anthropic schema-complexity, compilation, union, or optional-parameter rejection wording. |
...
| 32 | Closes the scope or expression introduced here: Detects oversized Anthropic prompts, context, requests, PDFs, or page counts. |
...
| 35 | Closes the scope or expression introduced here: Detects unreadable, invalid, encrypted, password-protected, or corrupt PDF wording from Anthropic. |
...
| 38 | Closes the scope or expression introduced here: Detects unsupported structured-output, document, image, tool-choice, or thinking features reported by Anthropic. |
| 39 | Leaves an unrecognized Anthropic failure unclassified for the caller's generic fallback. |
| 40 | Closes the scope or expression introduced here: Defines Anthropic-specific message classification into fixed application errors. |
| 41 | Blank line separating the surrounding declarations, statements, or document blocks. |
| 42 | Existing explanatory comment: Google RequestError keeps its response in data, and may retain a key-bearing URL. |
...
| 50 | Closes the scope or expression introduced here: Detects unsupported structured-output, document, image, tool-choice, or thinking features reported by Anthropic. |
...
| 54 | Closes the scope or expression introduced here: Detects unsupported structured-output, document, image, tool-choice, or thinking features reported by Anthropic. |
...
| 58 | Closes the scope or expression introduced here: Detects invalid or overly complex Google response schemas and nesting limits. |
...
| 61 | Closes the scope or expression introduced here: Detects unreadable, invalid, encrypted, password-protected, or corrupt Google PDF input reports. |
...
| 64 | Closes the scope or expression introduced here: Detects Google request, PDF, payload, page-count, token-count, or context limits. |
...
| 67 | Closes the scope or expression introduced here: Detects unsupported Google schema, output MIME type, PDF, image, tool, or function-calling capabilities. |
| 68 | Leaves an unrecognized Google failure for the outer generic classification. |
| 69 | Closes the scope or expression introduced here: Defines Google-specific error classification using nested response data and HTTP status. |
| 70 | Blank line separating the surrounding declarations, statements, or document blocks. |
| 71 | Existing explanatory comment: xAI uses OpenAI-compatible errors; inspect their details only to choose fixed messages. |
...
| 80 | Closes the scope or expression introduced here: Detects the invalid_api_key code or textual invalid, incorrect, or expired xAI key errors. |
...
| 84 | Closes the scope or expression introduced here: Detects the invalid_api_key code or textual invalid, incorrect, or expired xAI key errors. |
| 85 | Detects xAI's explicit unavailable-model error code. |
| 86 | Returns a fixed HTTP 502 Grok model-access error. |
| 87 | Closes the scope or expression introduced here: Detects xAI's explicit unavailable-model error code. |
...
| 91 | Closes the scope or expression introduced here: Detects xAI JSON schema invalidity, complexity, compilation, or nesting limits. |
...
| 94 | Closes the scope or expression introduced here: Detects xAI request, document-image, input-token, or context size limits. |
...
| 97 | Closes the scope or expression introduced here: Detects unsupported xAI structured-output, image/vision, tool, or function-calling capabilities. |
| 98 | Leaves an unrecognized xAI failure for the outer generic classifier. |
| 99 | Closes the scope or expression introduced here: Defines xAI-specific classification while keeping OpenAI-compatible raw error details private. |
| 100 | Blank line separating the surrounding declarations, statements, or document blocks. |
| 101 | Existing explanatory comment: Classify provider failures without logging keys, document content, or raw responses. |
...
| 111 | Closes the scope or expression introduced here: Applies xAI detail classification only for the selected client, quota, and rate-limit status codes. |
...
| 114 | Closes the scope or expression introduced here: Starts provider-specific handling for xAI failures. |
...
| 120 | Closes the scope or expression introduced here: Applies Google's detailed classifier to selected request, access, and rate-limit status codes. |
...
| 125 | Closes the scope or expression introduced here: Recognizes Google input, configuration, invalid-tool, and missing-tool-call exceptions as request-preparation failures. |
...
| 128 | Closes the scope or expression introduced here: Starts provider-specific handling for Google failures. |
...
| 137 | Closes the scope or expression introduced here: Handles remaining HTTP 400 and 422 request rejections. |
...
| 141 | Closes the scope or expression introduced here: Recognizes standard abort/timeout names and the OpenAI SDK connection-timeout class. |
| 142 | Falls back to a fixed HTTP 500 preparation failure without exposing original exception details. |
| 143 | Closes the scope or expression introduced here: Exports conversion of any failure to a public AppError, using OpenAI labels when no provider is supplied. |
