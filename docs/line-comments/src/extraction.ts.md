# Line explanations: src/extraction.ts

Source: [src/extraction.ts](../../../src/extraction.ts). Numbers refer to the original file before comments were added.
...
| ---: | --- |
| 1 | Imports ChatOpenAI from @langchain/openai for the OpenAI chat model adapter. |
| 2 | Imports ChatAnthropic from @langchain/anthropic for the Anthropic chat model adapter. |
| 3 | Imports compile-time types ChatGoogle from @langchain/google for the Google Gemini chat model adapter. |
| 4 | Imports compile-time types ChatXAI from @langchain/xai for the xAI Grok chat model adapter. |
| 5 | Imports HumanMessage from @langchain/core/messages for typed human/tool messages and provider-compatible content. |
| 6 | Imports extractionSchema, type AiProvider, type ExtractedOrder, type JobPayload from ./domain.js for shared validated application data, business rules, defaults, and domain types. |
| 7 | Imports APPRAISAL_PRODUCT_INSTRUCTIONS, CONTACT_ROLE_INSTRUCTIONS, SALE_PRICE_INSTRUCTIONS from ./system-prompt.js for shared appraisal workflow, contact, sale-price, and product instructions. |
| 8 | Imports anthropicExtractionSchema, normalizeAnthropicExtraction from ./extraction-schema.js for Anthropic wire-schema adaptation and canonical value normalization. |
| 9 | Imports createGoogleModel from ./google.js for the explicit-key Google model transport factory. |
| 10 | Imports createXaiModel from ./xai.js for the explicit-key xAI model transport factory. |
| 11 | Imports renderPdfPages from ./pdf-pages.js for local PDF-to-page-image rendering for Grok input. |
...
| 15 | Existing explanatory comment: Explicit credentials and official endpoints keep each job with its selected provider. |
...
| 22 | Closes the scope or expression introduced here: Selects the Anthropic SDK path when that provider was requested. |
...
| 25 | Closes the scope or expression introduced here: Exports model creation with explicit credentials, model ID, and an OpenAI provider default. |
| 26 | Blank line separating the surrounding declarations, statements, or document blocks. |
| 27 | Existing explanatory comment: Retry only an explicitly unsupported output format, never auth, quota, or model failures. |
...
| 35 | Closes the scope or expression introduced here: Uses Google-specific names when matching unsupported output-format errors. |
...
| 37 | Closes the scope or expression introduced here: Defines a narrow detector for output-format errors that permit a same-model structured-output fallback. |
...
| 48 | Existing explanatory comment: Gemini inlineData has no filename; identify each source before its PDF. |
...
| 51 | Closes the scope or expression introduced here: Selects a source-label-plus-file content block sequence for Google. |
...
| 54 | Closes the scope or expression introduced here: Defines construction of inline PDF content blocks for providers that accept native PDFs. |
...
| 70 | Existing explanatory comment: Grok receives labeled rendered pages; other providers receive native inline PDFs. |
| 71 | Appends the prepared native PDF blocks or locally rendered page-image blocks to the request content. |
| 72 | Closes the scope or expression introduced here: Begins the human message's extraction instructions and document content array. |
...
| 76 | Existing explanatory comment: Native JSON output also works with Claude models whose thinking is on by default. |
...
| 82 | Existing explanatory comment: Older tool-capable models can use the same schema and selected model via a tool call. |
...
| 84 | Closes the scope or expression introduced here: Handles failures of the initial structured-output invocation. |
...
| 86 | Closes the scope or expression introduced here: Exports document extraction using the selected model and cancellation signal, returning validated order facts. |
