# Line explanations: output/playwright/diagnose-login.mjs

Source: [output/playwright/diagnose-login.mjs](../../../../output/playwright/diagnose-login.mjs). Numbers refer to the original file before comments were added.
...
Later resource diagnostic mode: `--order-resources` records sanitized asset statuses/failures, waits for manual authenticated-page confirmation, opens the unfilled New Order page through the production guard, and reads only stylesheet count, body font and library-presence indicators. It does not fill or submit fields or call a model. New code has independent inline explanations without historical line numbers.
