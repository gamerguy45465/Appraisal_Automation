...
function convertStandardReasoningBlockToGeminiPart(block) {
	const ret = "reasoningContentBlock" in block && block.reasoningContentBlock ? convertStandardContentBlockToGeminiPart(block.reasoningContentBlock) : { text: block.reasoning };
	if (ret) ret.thought = true;
	return ret;
}
/**
...
function convertStandardContentBlockToGeminiPart(block) {
	function baseGeminiPart() {
		switch (block.type) {
			case "text": return { text: block.text };
			case "reasoning": return convertStandardReasoningBlockToGeminiPart(block);
			case "image":
			case "audio":
			case "text-plain":
	...
	if (ret) {
		if ("thoughtSignature" in block) ret.thoughtSignature = block.thoughtSignature;
	}
	return ret;
}
/**
...
function convertStandardContentMessageToGeminiContent(message, messages) {
	...
	if (_langchain_core_messages.AIMessage.isInstance(message) && message.tool_calls?.length) for (const toolCall of message.tool_calls) {
		const part = { functionCall: {
			name: toolCall.name,
			args: toolCall.args ?? {}
		} };
		if ("thoughtSignature" in toolCall) part.thoughtSignature = toolCall.thoughtSignature;
		parts.push(part);
	}
...
function convertLegacyPartToGeminiPart(item) {
	/**
	...
	*/
	...
	function baseGeminiPart() {
		if (typeof item === "string") return { text: item };
			...
			if (isMessageContentText(item)) return { text: item.text };
			else if ((0, _langchain_core_messages.isDataContentBlock)(item)) return (0, _langchain_core_messages.convertToProviderContentBlock)(item, geminiContentBlockConverter);
			...
			} else if (isMessageContentImageUrl(item)) return messageContentImageUrl(item);
			else if (isMessageContentMedia(item)) return messageContentMedia(item);
		}
		return item;
	}
	const ret = baseGeminiPart();
	const itemRecord = item;
	if ("thought" in itemRecord) ret.thought = itemRecord.thought;
	if ("thoughtSignature" in itemRecord) ret.thoughtSignature = itemRecord.thoughtSignature;
	return ret;
}
/**
